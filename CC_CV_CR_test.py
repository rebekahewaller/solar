...
def readVonPoint(serial):
    ...
    cmd[2] = 0x11
    resp = command(cmd, serial)
    von_volt = (resp[3] + (resp[4] << 8) + (resp[5] << 16) +
                (resp[6] << 24)) / 1000.00
    return von_volt
...
def readCCCurrent(serial):
    ...
    cmd[2] = 0x2B
    resp = command(cmd, serial)
    curr_in_CC = (resp[3] + (resp[4] << 8) + (resp[5] << 16) +
                  (resp[6] << 24)) / 10000.00
    return curr_in_CC
...
def readInputLevels(serial):
    ...
    cmd = [
        0
    ] * 26  # byte 3 to 6 = voltage (1 mV), byte 7 to 10 = current (0.1 mA)
    cmd[2] = 0x5F  # byte 11 to 14 = power (1 mW)
    resp = command(cmd, serial)
    volt_in = (resp[3] + (resp[4] << 8) + (resp[5] << 16) +
               (resp[6] << 24)) / 1000.00
    curr_in = (resp[7] + (resp[8] << 8) + (resp[9] << 16) +
               (resp[10] << 24)) / 10000.00
    power_in = (resp[11] + (resp[12] << 8) + (resp[13] << 16) +
                (resp[14] << 24)) / 1000.00
...
def opencircuit(opv_sample, log_file, serial):
    ...
    write_data(
        log_file,
        [opv_sample,
         strftime("%a, %d %b %Y %H:%M:%S"), oc[0], oc[1], oc[2]
...
def readCVVoltage(serial):
    ...
    cmd[2] = 0x2D
    resp = command(cmd, serial)
    volt_in_CV = (resp[3] + (resp[4] << 8) + (resp[5] << 16) +
                  (resp[6] << 24)) / 1000.00
    return volt_in_CV
...
def curve(voc, opv_sample, log_file, serial):
    ...
    while volt_step > 0.5:
        ...
        write_data(log_file, [
            opv_sample,
            strftime("%a, %d %b %Y %H:%M:%S"), curve_pt[0], curve_pt[1],
            curve_pt[2]
        ])
...
def shortcircuit(opv_sample, log_file, serial):
    ...
    setCVVoltage(
        0.1,
        serial)  # Set CV mode voltage to 0.1 volts (nearest value to 0 volts)
    time.sleep(1)
    sc = readInputLevels(serial)
    write_data(
        log_file,
        [opv_sample,
         strftime("%a, %d %b %Y %H:%M:%S"), sc[0], sc[1], sc[2]])
    jsc = sc[1]
    print(jsc)
    return jsc
...
def main():
    ...
    data_file(['opv_sample', 'time', 'volts', 'current', 'power'],
              log_file_postfix='LOAD')
    time.sleep(1)
    ...
    s.enter(
        15, 1, sweep, (opv_sample, serial)
    )  # need different function to only execute after 30 seconds has elapsed since function was complete
    s.run()
